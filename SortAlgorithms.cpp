...
int SortAlgorithms::quickSort (PagedArray array, int low, int high){
    PagedArray* arrayPtr = (PagedArray*)malloc(sizeof(array));
    *arrayPtr = array;
    quickSortAux(*arrayPtr, low, high);
...
int SortAlgorithms::insertionSort(PagedArray array, int size) {
    ...
    for (int i = 0; i < size; i++) {
        ...
        while (index >= 0 && temp <= *array[index]){
            //assign(array[index+1],array[index]);
            *array[index+1] = *array[index];
            index--;
        }
        //assign(array[index+1] ,temp);
        *array[index+1] = temp;
    }
